                    ...
                    if (additionalLogging)
                    {
                        fsm.InsertFsmMethod(s.Name, () =>
                        {
                            ...
                            for (j = 0; j < origFloatVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[FloatVariables] - '{origFloatVariables[j].Name}': '{origFloatVariables[j].Value}'");
                            for (j = 0; j < origIntVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[IntVariables] - '{origIntVariables[j].Name}': '{origIntVariables[j].Value}'");
                            for (j = 0; j < origBoolVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[BoolVariables] - '{origBoolVariables[j].Name}': '{origBoolVariables[j].Value}'");
                            for (j = 0; j < origStringVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[StringVariables] - '{origStringVariables[j].Name}': '{origStringVariables[j].Value}'");
                            for (j = 0; j < origVector2Variables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[Vector2Variables] - '{origVector2Variables[j].Name}': '({origVector2Variables[j].Value.x}, {origVector2Variables[j].Value.y})'");
                            for (j = 0; j < origVector3Variables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[Vector3Variables] - '{origVector3Variables[j].Name}': '({origVector3Variables[j].Value.x}, {origVector3Variables[j].Value.y}, {origVector3Variables[j].Value.z})'");
                            for (j = 0; j < origColorVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[ColorVariables] - '{origColorVariables[j].Name}': '{origColorVariables[j].Value}'");
                            for (j = 0; j < origRectVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[RectVariables] - '{origRectVariables[j].Name}': '{origRectVariables[j].Value}'");
                            for (j = 0; j < origQuaternionVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[QuaternionVariables] - '{origQuaternionVariables[j].Name}': '{origQuaternionVariables[j].Value}'");
                            for (j = 0; j < origGameObjectVariables.Length; j++)
                                Logger.Log($"[{fsm.gameObject.name}]:[{fsm.FsmName}]:[GameObjectVariables] - '{origGameObjectVariables[j].Name}': '{origGameObjectVariables[j].Value}'");
                        }, i + 1);
                    }
                }
            }
